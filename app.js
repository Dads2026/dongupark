document.addEventListener("DOMContentLoaded", () => {
    ...
    function openBottomSheet(node) {
        ...
        rewards[0].innerText = `+${node.energy} ENERGY`;
        rewards[1].innerText = `+${node.flow} FLOW`;
        
        // Change colors based on type
        const typeEl = document.getElementById('node-type');
        typeEl.style.color = `var(--accent-${node.type === 'zen' ? 'orange' : node.type === 'social' ? 'blue' : 'neon'})`;
